...
export const deletePost = async (req, res)=>{
    try {
        ...
        if(post.user.toString() !== req.user._id.toString()) return res.status(404).json({ message:"you are not allowed to delete this post" });
        if(post.img){
...
export const getAllLike = async (req, res) =>{
    ...
    try {
        ...
        console.log("Error in getAllLike controller: ", e);
        res.status(500).json({ error: "Internal server error" });
    };
};
